...
typedef struct pdp_imgloader_struct
{
    ...
    t_float     x_blend;
    t_int       quality;   // quality forces an additional yuv->rgb conversion in yuv mode
...
static void pdp_imgloader_load(t_pdp_imgloader *x, t_symbol *filename, t_floatarg fx, t_floatarg fy)
{
  Imlib_Load_Error imliberr;
   ...
   DATA8 tablas[768];
   DATA8 *redt,*greent,*bluet,*alphat;
   redt = &tablas[0];
   greent = &tablas[256];
   bluet = &tablas[512];
}
...
static void pdp_imgloader_estirar(t_pdp_imgloader *x, t_floatarg festirar )
{
   if ( ( festirar >= 0.0 ) )
   {
     x->b_estirar = (int)festirar;
   }
}
...
static void pdp_imgloader_process_rgba(t_pdp_imgloader *x)
{
    ...
    memcpy( newdata, data, (x->x_vsize*4 ));
    Imlib_Image im_packet = imlib_create_image_using_data( x->x_vwidth, x->x_vheight, newdata );
   if ( im_packet == NULL )
   {
      post( "pdp_text : severe error : could not allocate image !!" );
      return;
   }
   imlib_context_set_image(im_packet);
   draw_rgb_image(x);
   imlib_context_set_image(im_packet);
...
static void draw_rgb_image(t_pdp_imgloader *x)
{
   ...
   int imblend = x->x_blend*255;
   for (i=imblend;i<256;i++)
   {
	   alphat[i] = imblend;
   }
   ...
   if (x->b_estirar)
       imlib_blend_image_onto_image(image_buf,0,0,0,x->x_iwidth,x->x_iheight,x->x_xoffset,x->x_yoffset,x->x_xoffset+x->x_vwidth,x->x_yoffset+x->x_vheight);
   else
...
static void pdp_imgloader_process_yv12(t_pdp_imgloader *x)
{
	...
	//pdp_llconv(data, RIF_YVU__P411_S16, imdata, RIF_BGRA_P____U8, x->x_vwidth, x->x_vheight);
   ...
   if ( (header = pdp_packet_header(x->x_packet0))
	...
	switch(pdp_packet_header(x->x_packet0)->info.image.encoding)
	  ...
	  case PDP_IMAGE_GREY:
	    ...
	    break;
	  /*case PDP_IMAGE_RGBA8:
            x->x_packet1 = pdp_packet_clone_rw(x->x_packet0);
            pdp_queue_add(x, pdp_imgloader_process_rgba, pdp_imgloader_sendpacket, &x->x_queue_id);
	    break;*/
	  default:
...
static void pdp_imgloader_input_0(t_pdp_imgloader *x, t_symbol *s, t_floatarg f)
{
	...
	switch(pdp_packet_header((int)f)->info.image.encoding)
	  ...
	  break;
	  /*case PDP_IMAGE_RGBA8:
          x->x_dropped = pdp_packet_convert_ro_or_drop(&x->x_packet0, (int)f, pdp_gensym("image/rgba/*") );
	  break;*/
	}
...
void pdp_imgloader_setup(void)
{
    ...
    class_addmethod(pdp_imgloader_class, (t_method)pdp_imgloader_estirar, gensym("estirar"),  A_DEFFLOAT, A_NULL);
